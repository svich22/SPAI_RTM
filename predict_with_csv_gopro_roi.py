...
def draw_roi(image):
    ...
    for i in range(0, 4):
        cv2.setMouseCallback('XYZ', click_event)
        cv2.waitKey(0)
        if len(roi_cords) >= 4:
            cv2.setMouseCallback('XYZ', lambda *args: None)
    print(roi_cords)
    roi_taken = True
...
cap = cv2.VideoCapture(0)  # for go pro use 1 for webcam use 0
