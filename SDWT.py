'''
Description - 
Shuttle detection with trajectory mapping
'''
...
def draw_roi(image, cords):
    ...
    if cords == (0, 0):
        n_frames += 1
        print(" >>> ")
        image = cv2.polylines(image, [np.array(dets)], 1, (0,255,255))
        return image
 
    # Draw traj on img
    visibility=True
    cv2.putText(image, "IN", POSITION,
                cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 247, 0), 2)
    if len(dets) < trajectory_limit:
        if visibility:
            dets.append((pt[0],pt[1]))
    else:
        # Perform FIFO Queue
        if visibility:
            dets.pop(0)
    ...
    if in_roi:
        ...
        image = cv2.polylines(image, [np.array(dets)], 1, (0,255,255))
        print(" <<< ")
        cv2.putText(image, "OUT", POSITION,
                    cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 247, 0), 3)
        n_frames += 1
        return image
